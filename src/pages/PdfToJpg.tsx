...
import {
  Container, Box, Typography, Card, CardContent, Button, Alert, LinearProgress, Grid, Paper, Chip, Slider, FormControl, InputLabel, Select, MenuItem,
} from '@mui/material';
    ...
    try {
      ...
      for (let pageNum = 1; pageNum <= totalPages; pageNum++) {
        try {
          ...
          // Create a simple representation (this is a simplified version)
          ctx.fillStyle = '#ffffff';
          ctx.fillRect(0, 0, canvas.width, canvas.height);
          
          ctx.fillStyle = '#000000';
          ctx.font = `${16 * scale}px Arial`;
          ctx.fillText(`Page ${pageNum}`, 20 * scale, 30 * scale);
          ctx.fillText(`Width: ${width}`, 20 * scale, 60 * scale);
          ctx.fillText(`Height: ${height}`, 20 * scale, 90 * scale);
